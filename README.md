...
Trong đó: $g_{angle} = -m \cdot \hat{z}_i$ và $g_{add} = m \cdot \hat{z}_i + m$. Các thành phần này đóng vai trò margin thích nghi theo chất lượng mẫu.
...
Kết quả thực nghiệm khẳng định rằng việc sử dụng AdaFace giúp mô hình giữ được tỷ lệ chấp nhận đúng (TAR) rất cao ngay cả khi tỷ lệ chấp nhận sai (FAR) bị giới hạn ở mức rất thấp ($10^{-4}$). Điều này chứng minh khả năng phân biệt cực tốt của mô hình đối với các cặp ảnh khó và ảnh chất lượng thấp.
